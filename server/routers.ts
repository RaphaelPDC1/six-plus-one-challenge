  ...
  challenge: router({
    ...
    uploadProof: protectedProcedure
      .input(z.object({
        ...
        dataUrl: z.string().max(70_000_000).regex(/^data:(image\/(png|jpeg|webp)|video\/[a-z0-9.+\-]+);base64,[A-Za-z0-9+/=]+$/i, "Proof media must be a PNG, JPG, WEBP, MP4, MOV, or WEBM data URL."),
      }))
      .mutation(async ({ ctx, input }) => {
        const participant = await getOrCreateParticipant(ctx.user);
