...
describe("PWA and mobile layout refinements", () => {
  it("uses the game name and PNG app icons for saved home-screen shortcuts", () => {
  ...
  it("keeps Log Today stats below Must Do and uses the floating safe-area mobile nav", () => {
    ...
    expect(source).toContain("Browsers control installation, so the app cannot force auto-save");
  });
});
