...
function replaceOnce(filePath, find, replace) {
  ...
  if (!source.includes(find)) {
    throw new Error(`Pattern not found in ${filePath}: ${find.slice(0, 120)}`);
  }
  writeFileSync(filePath, source.replace(find, replace));
}

const root = "/home/ubuntu/six-plus-one-challenge";
const cookiesPath = `${root}/server/_core/cookies.ts`;
replaceOnce(
  cookiesPath,
...
const testPath = `${root}/client/src/pages/Home.onboarding.test.tsx`;
replaceOnce(
  testPath,
