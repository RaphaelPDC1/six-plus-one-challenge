...
OUT = Path('/home/ubuntu/six-plus-one-challenge/handoff/overview-page-long-screenshot-native.png')
PROFILE = Path('/tmp/sixone-overview-cdp-profile')
PORT = 9333
WIDTH = 894
HEIGHT = 900
LOCAL_STORAGE = {
...
def main():
    ...
    try:
        ...
        try:
            ...
            time.sleep(4)
            # Ensure Overview tab is selected if the app routes via state rather than URL.
            cdp.call('Runtime.evaluate', {'expression': "Array.from(document.querySelectorAll('button')).find(b => b.textContent && b.textContent.trim().toUpperCase()==='OVERVIEW')?.click();", 'awaitPromise': True})
            time.sleep(1)
            # Hide the preview warning strip for a cleaner handoff screenshot if present.
            cdp.call('Runtime.evaluate', {'expression': "Array.from(document.querySelectorAll('*')).filter(el => (el.textContent||'').includes('Preview mode') && (el.textContent||'').includes('not live')).forEach(el => el.style.display='none');", 'awaitPromise': True})
            ...
            print(OUT)
            print(f'{WIDTH}x{int(content["height"])}')
        finally:
            cdp.close()
    finally:
        proc.terminate()
