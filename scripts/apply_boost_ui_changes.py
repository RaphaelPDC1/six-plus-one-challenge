...
def must_replace(find: str, replace: str):
    global text
    if find not in text:
        raise RuntimeError('Missing marker: ' + find[:100])
    text = text.replace(find, replace, 1)

def replace_between(start: str, end: str, replace: str):
    global text
    s = text.find(start)
    if s < 0:
        raise RuntimeError('Missing start: ' + start[:100])
    e = text.find(end, s + len(start))
    if e < 0:
        raise RuntimeError('Missing end: ' + end[:100])
    text = text[:s] + replace + text[e + len(end):]

must_replace('function Overview({ snapshot }: { snapshot: Snapshot }) {\n  const [selected, setSelected] = useState<any>(null);\n', '''function getBoostToneClass(tone?: string) {
...
must_replace('  const currentParticipant = insights.find((participant: any) => String(participant.id) === String(snapshot?.participant?.id)) ?? rankedInsights[0];\n', '''  const currentParticipant = insights.find((participant: any) => String(participant.id) === String(snapshot?.participant?.id)) ?? rankedInsights[0];
...
replace_between('  const boosted = [...ranked].sort((a: any, b: any) => b.boostScore - a.boostScore || b.recentPointGain - a.recentPointGain).slice(0, 4);\n', '  return (\n', '''  const leaderPoints = Number(ranked[0]?.totalPoints ?? 0);
  const activeBoosts = snapshot?.activeBoosts ?? [];
...
must_replace('            <h3 className="mt-2 text-2xl font-black uppercase leading-none tracking-[-0.07em] text-white">Three slots. Earned, not gamed.</h3>\n', '            <h3 className="mt-2 text-2xl font-black uppercase leading-none tracking-[-0.07em] text-white">Rotating slots. Earned, not gamed.</h3>\n')
must_replace('          <span className="shrink-0 rounded-full border border-[#2ECC71]/55 bg-black/45 px-3 py-1.5 text-[9px] font-black uppercase tracking-[0.16em] text-[#2ECC71]">Live rules</span>\n', '          <span className="shrink-0 rounded-full border border-[#2ECC71]/55 bg-black/45 px-3 py-1.5 text-[9px] font-black uppercase tracking-[0.16em] text-[#2ECC71]">Day {currentDay}</span>\n')
must_replace('            const toneClass = slot.tone === "green" ? "border-[#2ECC71]/55 bg-[#082016] text-[#2ECC71]" : slot.tone === "red" ? "border-[#C0392B]/55 bg-[#190B0A] text-[#FFB3A8]" : "border-[#C8A96E]/55 bg-[#16130B] text-[#C8A96E]";\n', '            const toneClass = getBoostToneClass(slot.tone);\n')
must_replace('                  <span className="text-[9px] font-black uppercase tracking-[0.16em]">Slot {index + 1}</span>\n', '                  <span className="text-[9px] font-black uppercase tracking-[0.16em]">{slot.icon} · Slot {slot.slot ?? index + 1}</span>\n')
