...
const start = source.indexOf('function Overview({ snapshot }: { snapshot: Snapshot }) {');
const end = source.indexOf('const COMPLIANCE_RULE_LABELS =', start);
if (start === -1 || end === -1) {
  throw new Error('Could not locate Overview block boundaries');
}

const replacement = String.raw`function InsightPill({ label, value, tone = "gold" }: { label: string; value: string | number; tone?: "gold" | "red" | "green" | "purple" | "white" }) {
  const tones = {
