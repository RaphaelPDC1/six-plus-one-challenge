...
schema = replace_once(schema, '''  source: varchar("source", { length: 120 }).default("landing").notNull(),
  approvedByUserId: int("approvedByUserId"),''', '''  source: varchar("source", { length: 120 }).default("landing").notNull(),
  ...
  approvedByUserId: int("approvedByUserId"),''', 'signup request questionnaire fields')
schema = replace_once(schema, '''  monzoPaymentLink: text("monzoPaymentLink"),
  currentStreak: int("currentStreak").default(0).notNull(),''', '''  monzoPaymentLink: text("monzoPaymentLink"),
  profilePhotoUrl: text("profilePhotoUrl"),
  profilePhotoKey: text("profilePhotoKey"),
  primaryGoal: varchar("primaryGoal", { length: 220 }),
  biggestObstacle: text("biggestObstacle"),
  ...
  currentStreak: int("currentStreak").default(0).notNull(),''', 'participant onboarding fields')
write('drizzle/schema.ts', schema)

db = read('server/db.ts')
db = replace_once(db, 'import { ENV } from "./_core/env";', 'import { ENV } from "./_core/env";\nimport { storagePut } from "./storage";', 'storage import')
db = replace_once(db, '''export async function createSignupRequest(email: string, source = "landing") {
...
''', 'onboarding helpers')
db = replace_once(db, '''  await db.insert(participants).values({
    userId: user.id,
...
db = replace_once(db, 'export async function updateParticipantProfile(userId: number, input: { displayName: string; whatsappName?: string; monzoPaymentLink?: string }) {', 'export async function updateParticipantProfile(userId: number, input: { displayName: string; whatsappName?: string; monzoPaymentLink?: string; primaryGoal?: string; biggestObstacle?: string; trainingLevel?: string; motivationStyle?: string }) {', 'profile input type')
db = replace_once(db, '''    monzoPaymentLink: input.monzoPaymentLink || DEFAULT_MONZO_PAYMENT_LINK,
  }).where(eq(participants.id, participant.id));''', '''    monzoPaymentLink: input.monzoPaymentLink || DEFAULT_MONZO_PAYMENT_LINK,
    ...
    onboardingCompleted: true,
  }).where(eq(participants.id, participant.id));''', 'profile update fields')
db = replace_once(db, '''export async function getAppSnapshot(userId: number, role: "admin" | "user" = "user") {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  const participant = await getOrCreateParticipant({ id: userId, name: null, email: null });
  await seedRewardsIfEmpty();''', '''export async function getAppSnapshot(userId: number, role: "admin" | "user" = "user", email: string | null = null) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  ...
  await seedRewardsIfEmpty();''', 'snapshot onboarding gate')
db = replace_once(db, '''  return {
    challenge: { currentDay: getCurrentChallengeDay(), totalDays: 50, monzoPaymentLink: DEFAULT_MONZO_PAYMENT_LINK },''', '''  return {
    accessState: { status: "ready" as const, reason: onboarding.reason },
    challenge: { currentDay: getCurrentChallengeDay(), totalDays: 50, monzoPaymentLink: DEFAULT_MONZO_PAYMENT_LINK },''', 'snapshot access state')
write('server/db.ts', db)

routers = read('server/routers.ts')
routers = replace_once(routers, '  createSignupRequest,', '  completeOnboarding,\n  createSignupRequest,', 'complete import')
routers = replace_once(routers, 'return getAppSnapshot(ctx.user.id, ctx.user.role);', 'return getAppSnapshot(ctx.user.id, ctx.user.role, ctx.user.email);', 'snapshot email arg')
routers = replace_once(routers, '''        monzoPaymentLink: z.string().max(2000).optional().default(""),
...
routers = replace_once(routers, '''      .mutation(async ({ ctx, input }) => {
        return updateParticipantProfile(ctx.user.id, input);
      }),

    submitMyDay: protectedProcedure.input(dayLogInput).mutation(async ({ ctx, input }) => {''', '''      .mutation(async ({ ctx, input }) => {
        return updateParticipantProfile(ctx.user.id, input);
      }),
    ...
    submitMyDay: protectedProcedure.input(dayLogInput).mutation(async ({ ctx, input }) => {''', 'complete onboarding procedure')
write('server/routers.ts', routers)

home = read('client/src/pages/Home.tsx')
home = replace_once(home, '  Crown,', '  Camera,\n  Crown,', 'camera icon import')
home = replace_once(home, '''function SignupAccessForm() {''', '''function LogoMark({ compact = false }: { compact?: boolean }) {
  return (
...
function SignupAccessForm() {''', 'logo/profile components')
home = replace_once(home, '''        <nav className="flex items-center justify-between border-b border-[#2A2A2A] pb-5">
          <div>
          ...
          </div>''', '''        <nav className="flex items-center justify-between gap-4 border-b border-[#2A2A2A] pb-5">
          ...
          </div>''', 'landing top logo')
home = home.replace('''<span className="grid h-12 w-12 place-items-center border border-[#C8A96E] text-sm font-black text-[#C8A96E]">{p.avatarInitials}</span>''', '''<ProfilePhoto participant={p} className="h-12 w-12" />''')
home = home.replace('''<span className="grid h-11 w-11 place-items-center border border-[#C8A96E] text-xs font-black text-[#C8A96E]">{owner?.avatarInitials ?? "?"}</span>''', '''<ProfilePhoto participant={owner} className="h-11 w-11" />''')
home = replace_once(home, '''function AdminPanel({ snapshot, refetch }: { snapshot: Snapshot; refetch: () => void }) {''', '''function OnboardingGate({ user, refetch }: { user: any; refetch: () => void }) {
...
function AdminPanel({ snapshot, refetch }: { snapshot: Snapshot; refetch: () => void }) {''', 'onboarding gate')
home = replace_once(home, '''                  <p className="mt-2 text-[10px] font-black uppercase tracking-[0.18em] text-[#777]">{request.source} · {new Date(request.createdAt).toLocaleString()}</p>''', '''                  <p className="mt-2 text-[10px] font-black uppercase tracking-[0.18em] text-[#777]">{request.source} · {new Date(request.createdAt).toLocaleString()}</p>
                  ...
                  {(request.trainingLevel || request.motivationStyle) && <p className="mt-2 text-[10px] font-black uppercase tracking-[0.16em] text-[#777]">{request.trainingLevel} · {request.motivationStyle}</p>}''', 'admin questionnaire visibility')
home = replace_once(home, '''  if (!isAuthenticated) return <Landing />;

  const visibleTabs = tabs.filter(tab => tab.key !== "admin" || user?.role === "admin");''', '''  if (!isAuthenticated) return <Landing />;
  if (snapshot?.accessState?.status === "questionnaire_required") return <OnboardingGate user={user} refetch={snapshotQuery.refetch} />;

  const visibleTabs = tabs.filter(tab => tab.key !== "admin" || user?.role === "admin");''', 'onboarding route')
home = home.replace('''<div className="grid h-11 w-11 place-items-center border border-[#C8A96E] font-black text-[#C8A96E]">6+1</div>''', '''<LogoMark compact />''')
write('client/src/pages/Home.tsx', home)

test = read('server/challengeRouterFlow.test.ts')
test = replace_once(test, '  createSignupRequest: vi.fn(),', '  completeOnboarding: vi.fn(),\n  createSignupRequest: vi.fn(),', 'test mock complete')
test = replace_once(test, '''  it("routes founder signup approval and rejection through admin helpers", async () => {''', '''  it("routes personalized onboarding through the completion helper", async () => {
  ...
  it("routes founder signup approval and rejection through admin helpers", async () => {''', 'test onboarding route')
write('server/challengeRouterFlow.test.ts', test)
print('v4 onboarding changes applied')
